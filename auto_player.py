...
def random_frequency(frequency):
    ...
    :return:
    """
    frequency = random.randint(1, frequency)
    return frequency


# 随机延迟点击，防止游戏外挂检测，延迟时间范围为【x, y】秒之间
def random_delay(x=0.2, y=0.6):
    """
    随机延迟点击，防止游戏外挂检测，延迟时间范围为【x, y】秒之间
    :param x:
    :param y:
...
def find_touch_any(target_list, tap=True, randomtouch=True, frequency=4):
    """
    寻找并点击,找到返回目标名，未找到返回NONE
    :param target_list:
    :param tap:
    ...
    for target in target_list:
        ...
        if pts:
            ...
            if tap:
                if randomtouch:
                    i = 0
                    frequency_all = random_frequency(frequency)
                    while i < frequency_all:
                        xx = pts[0]
                        xx = random_offset(xx, w, h)
                        touch(xx)
                        print("第", (i + 1), "次点击")
